...
const ProductFilter = () => {
    ...
    const handleFieldChange = useCallback((line: string) => (_: boolean) => {
        const lines: string[] = filter?.lines ?? [];
        setFilters({ ...filter, lines: lines.includes(line) ? lines.filter((l) => l !== line) : [...lines, line] });
    }, [filter, setFilters]);
    ...
    return (
        <div>
            <button className="cursor-pointer" onClick={handleShowFilters}>Filter</button>
            <div className="relative">
                <Popup isOpen={isFiltersVisible} onClickOutside={() => setFiltersVisible(false)}>
                    <div className="p-4">
                        ...
                        {isLoading && <Loader />}
                        {fields.map(field => <CheckboxField key={field.value} label={field.label} checked={field.checked} onChange={handleFieldChange(field.value)} />)}
                        <div className="text-left mt-2">
                            <a className="text-red-500 cursor-pointer" onClick={handleResetFilters}>Reset</a>
                        </div>
                    </div>
                </Popup>
            </div>
